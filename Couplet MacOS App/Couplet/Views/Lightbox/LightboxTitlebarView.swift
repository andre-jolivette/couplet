...
///
/// Layout uses a ZStack so the pair counter is centered to the full window width
/// independently of the back button and score chips. The HStack holds only the
/// leading and trailing controls; the counter floats above it, centered to the
/// ZStack's full frame (= full window width).
struct LightboxTitlebarView: View {
    ...
    var body: some View {
        ...
        ZStack {
            // Pair counter — centered to the full window width.
            if vm.pairCount > 0 {
            ...
            // Back button (leading) and score/info group (trailing).
            // frame + contentShape are on the Button, not the label, so the entire
            // rendered area — including padding around the icon/text — is clickable.
            HStack(spacing: 0) {
