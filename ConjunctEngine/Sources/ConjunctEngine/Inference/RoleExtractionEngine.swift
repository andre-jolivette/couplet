...
- claims: array of short statements DISPLAYED as text or signs in the image. CRITICAL: do NOT copy the sign's words verbatim — translate each sign to the ACTION OR IDEA IT DEMANDS, as a 1-2 word concept. "SMILE you're beautiful" -> ["smile"]; "SEE SOMETHING SAY SOMETHING" -> ["danger"] (it is a public-safety warning about danger); "SHOOT HERE" -> ["shoot"]; "NO PARKING" -> ["parking"]. Never output a full sign sentence. Empty if no legible text.
- enacts: array of abstract CONCEPTS (emotions, intentions, social meanings, actions) the SUBJECT embodies — never plain object names. A smiling person -> ["smile"]; printed or drawn eyes/face that seem to watch -> ["watch"]; a person openly carrying a real gun -> ["danger","threat"] (NOT "gun"). Do not put object nouns like "gun", "taco", "car" here — those go in objects.
- subverts: array of concepts the subject physically PREVENTS, BLOCKS, COVERS, or CONTRADICTS — infer this from the physical description even when no text is present. A mouth covered by a mask/net/hand so it cannot smile or speak -> ["smile","speak"]; hands clamped over ears blocking sound -> []; a "keep out" gesture on a welcome mat -> ["welcome"]. Look hard for a body part or object that negates an expected action.
