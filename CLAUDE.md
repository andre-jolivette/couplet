...
**Anchor strip DB fetch must not LIMIT below the image's pair count** — `EngineController.fetchPairs` with `anchorImageID != nil` previously used `dbLimit = 200`. The `displayLimit` variable is only applied inside the `if anchorImageID == nil` branch, so the DB LIMIT was the sole cap for anchor queries. Hub images can have 400+ pairs; LIMIT 200 silently dropped the bottom half under the DB ORDER BY sort. Fix: unified `dbLimit = 2000` for all queries. See decision #86.
