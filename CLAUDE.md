...
| 66 | Composite scoring rethink — per-axis surfacing | Done. Two-phase fix: #75 (thematic boost fix + aesthetic topK pool) ensured one-axis-exceptional pairs enter the DB; #76 (axisScore) ensures they rank at the top of the default grid. Default sort is now "Best" (`axisScore = max(aesthetic, geometric×0.8, thematic) × temporalPenalty`). "Balanced" sort (weighted average) remains available. See decisions #75, #76. |
...
| 76 | axisScore display field + "Best" default sort | Done. `axisScore = max(aestheticScore, geometricScore×0.8, thematicScore) × temporalPenalty` added to `DisplayPair`. Default sort changed from "Composite" to "Best". "Composite" renamed "Balanced" in the sort picker. No DB changes. See decision #76. |
