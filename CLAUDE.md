...
8. **Pair scoring** — PairScorer, four-pool topK (composite top-150 + thematic top-50 + geometric top-5 + aesthetic top-5); two-phase: intra-folder first (blocking), cross-folder in background (cancellable) — decisions #34, #68, #75
...
| 66 | Composite scoring rethink — per-axis surfacing | **In progress.** #75 is a partial fix: corrects the thematic boost overreach and adds an aesthetic escape-hatch pool. The underlying problem remains — the weighted average composite still penalises pairs exceptional on one or two axes. Escape-hatch pools are workarounds, not a solution: they get specific pairs into the DB but the display grid still sorts primarily by composite, so strong one-axis pairs rank below mediocre-everywhere pairs once in the grid. The systemic fix requires replacing (or supplementing) the composite average with a ranking that rewards peak performance: `max(aesthetic, 0.8×geometric, thematic)` or per-axis grid sections. See decision #75 (partial) and decision #66 notes. Dedicated design session required — touches PairScorer.swift, IndexingEngine.swift, QueryService.swift, AppModels.swift, and the grid/sort display layer simultaneously. |
| 75 | Thematic boost fix + aesthetic topK escape hatch (partial #66) | Done as first step toward #66. (1) Thematic boost condition tightened: only fires when thematic is the dominant axis (`thematic >= max(aesthetic, geometric×0.8)`). (2) Aesthetic topK pool added (top-5 per image, ≥0.55 floor, `selectedFor = "aesthetic"`), symmetric to geometric pool (#68). (3) Modality detection in PairHelpers.swift updated. **What this does NOT fix:** pairs still sort in the grid by composite score, so even though strong one-axis pairs now enter the DB, they rank below mediocre-everywhere pairs if composite is low. The escape-hatch pools are a necessary prerequisite but not sufficient — the grid ranking needs to change too. Requires re-score. See decision #75. |
