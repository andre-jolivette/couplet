...
| 66 | Composite scoring rethink — per-axis surfacing | Done. Addressed by #75: (1) thematic boost now requires thematic to be the dominant axis; (2) aesthetic topK pool added. L1007802+punk now scores composite=0.584 (was 0.511 with the bad boost) and enters DB via both composite and aesthetic pools. See decision #75. |
| 75 | Composite scoring rethink: thematic boost fix + aesthetic topK | Done. Two changes: (1) thematic boost condition tightened to require `thematic >= max(aesthetic, geometric×0.8)` — fires only when thematic is dominant; (2) aesthetic topK pool added (top-5 per image, ≥0.55 floor, `selectedFor = "aesthetic"`), symmetric to the existing geometric pool. Modality detection in PairHelpers.swift updated to recognize `selectedFor == "aesthetic"`. Requires re-score. See decision #75. |
