...
| 46 | Complementary role scoring — two sides of the same phenomenon | The cluster system was designed for same-register resonance (both images evoke grief, both evoke bodily gesture). A separate mechanism is needed for cross-register complementarity: pairs where one image is the producer/agent and the other is the receiver/subject of the same phenomenon. Sender-receiver is one example (musician creates sound; woman blocks sound). But the pattern is broader: performer↔audience, creator↔creation, individual↔crowd (one person doing what many do), before↔after, labor↔leisure. The challenge is identifying these relationships in a principled way without enumerating every case. The visual diversity multiplier (#45) improves cross-context scoring but doesn't specifically reward complementary roles. Explore: (a) role annotations on clusters (producer/consumer/observer tags); (b) axis pairs — clusters that define opposite ends of the same dimension (sound_music ↔ sensory_overwhelm, power_dominance ↔ vulnerability_exposure); (c) caption-level LLM scoring for top-N composite candidates. Should be preceded by calibration data: after #45 re-index, check whether the musician+ears pair surfaces and whether remaining missed pairs are complementary-role failures or other issues. See decision #46. |
