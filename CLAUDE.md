...
| 73 | Dominant orientation opposition — opposing_diagonals submode | Done. `orientationOppositionScore()` uses stored 32-bin edge histogram: find dominant bin per image, fold to undirected [0,16), score perpendicularity as `sin(dist×π/16) × √(normPeakA × normPeakB)`. Wired into `directional = max(centroidScore, gazeScore, orientationScore)`. New submode `"opposing_diagonals"`. No re-index. See decision #73. |
