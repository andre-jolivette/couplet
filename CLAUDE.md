...
| 74 | Opposing diagonals — diagonalness factor | Done. Added `diag = |sin(undirBin × π/8)|` to `orientationOppositionScore()`. Score is now `sin(dist×π/16) × √(normPeakA×normPeakB) × √(diagA×diagB)`. Near-horizontal (0°) and near-vertical (90°) lines score 0 — "opposing diagonals" requires both lines to be diagonal, not just perpendicular. `_DSF2811` with near-horizontal line: diag≈0.38, score drops from ~0.78 to ~0.30. `_DSF1497` at 35°: diag≈0.92, retains high score. No re-index. See decision #74. |
