...
| 73 | Dominant orientation opposition — opposing_diagonals submode | Done. `orientationOppositionScore()` uses stored 32-bin edge histogram: find dominant bin per image, fold to undirected [0,16), score perpendicularity as `sin(dist×π/16) × √(normPeakA × normPeakB)`. Wired into `directional = max(centroidScore, gazeScore, orientationScore)`. New submode `"opposing_diagonals"`. No re-index. **Known ceiling:** `kEdgePeakedNorm=4.0` (p90) means ~10% of images saturate normPeak=1.0 and are indistinguishable by threshold. The histogram measures edge concentration but not aesthetic quality of the line angle — images with visually different diagonals may have identical peakedness scores. Not fixable with thresholding alone. See decision #73. |
