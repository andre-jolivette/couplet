...
| 85 | ThematicV2 — evaluate llama3.2:11b for better discrimination | 3B model responds to prompt constraints but cannot reliably calibrate confidence (binary 0.8/0.9) or consistently exclude color-based connections when captions mention color explicitly. 11B is the next lever. Infrastructure is ready — one-line model change in `ThematicScorerV2.init()`. Tradeoff: ~5–10× slower inference, ~45min–2hr for a 500-pair pass. Evaluate after 3B scoring has settled and a comparison set of pairs is available. |
