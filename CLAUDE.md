...
| 37 | Filter bar: left-divider-to-Composite gap | Gap between the left vertical divider and the "Composite" (Sort) picker looks wider than the matching gap on the right side (AllTones → right divider), even though the SwiftUI layout values are symmetric. Root cause is likely that NSPopUpButton (the AppKit control behind SwiftUI's `.menu` Picker) has asymmetric internal visual margins — its label text sits toward the right of its layout frame (dropdown arrow on far right), making the left edge appear to have extra whitespace. Multiple iterations of adjusting `HStack(spacing:)` and `Divider().padding(.horizontal:)` failed to close the gap. New approach needed — possibly: inspect the NSPopUpButton frame vs. its visible content rect at runtime and apply a compensating negative leading offset to the Sort picker; or replace the Picker with a custom NSPopUpButton wrapper that exposes the actual content insets. See `FilterBarView.swift` — Sort picker inside the inner `HStack(spacing: -8)` after the first `Divider().frame(height: 20)`. |
